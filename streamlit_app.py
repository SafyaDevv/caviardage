...
### SIDEBAR
url1 = "https://www.kaggle.com/datasets/aditeyabaral/blackout-poetry-dataset"
url2 = "https://github.com/lizadaly/blackout"

st.sidebar.write(
    f"_Original dataset posted by [Aditeya Baral on Kaggle]({url1}) "
    f"and generated using a modified version of [Liza Daly's Blackout Project]({url2})._"
)
...
page = st.sidebar.selectbox(
    ...
    ["Overview", 
     ...
     "Poem recommender",
     "Passages (input) vs Poems (output)",
...
image_file = "files/bard.png"
st.sidebar.image(image_file, caption="Illustration by Daniel Mróz for 'Cyberiada' by Stanisław Lem")
...
if page == "Overview":
    ...
    st.write("_A data exploration of the Blackout dataset. A dataset of 12k+ machine generated poems created by picking words from existing text passages._")
    st.write("")
    ...
    with col1:
        if st.button("Get a random poem 🎲"):
            ...
            st.write(f"<span style='font-size:20px; color: purple;'>'{text_poem}'</span><span style='font-size:15px;'>{id_poem}</span>",
            unsafe_allow_html=True) 

    with col2:
        st.warning("*Disclaimer*: These poems are generated by an algorithm and "\
        "may contain disturbing or inappropriate content.")
    ...
    with col2:
        ...
        st.write(clean_df_v2.describe()) 

### VISUALISATION HERE
if page == "Passages (input) vs Poems (output)":
...
if page == "Syntax & Sentiment Analysis":
    ...
    st.subheader("🔎 Find poems with specific \"part of speech\"")
    st.write("")
    # !! insert description of what a "part-of-speech" is
    ...
    #display poems
    st.write(f"Poems containing *(in any order)*: **{selection_label}**. There are ", len(filtered_poems), " poems to display.")
    st.write(filtered_poems[["poem", "poem-pos"]])

    st.write("")
    st.write("✦•····················•✦•······················•✦"*4)
    st.header("Find poems with specific sentiment expressed 🔎")
    st.write("")
    ...
    #display poems
    st.write(f"Poems with following filters applied: **{polarity_selection}** and **{subjectivity_selection}**. There are ", len(filtered_poems_s), " poems to display.")
    st.write(filtered_poems_s[["poem", "poem-polarity", "poem-subjectivity"]])
    
if page == "Themes exploration":

    st.subheader(
        "Explore poem themes identified through clustering analysis",)
    
    themes_fig = pio.read_json("files/poem_clusters.json")
    st.plotly_chart(themes_fig) #!! need to change labels font color to black

    st.write("_Please activate your browswer's light mode for better visibility of the hover labels._\n")

    st.subheader("Filter poems by theme")

    st.write("_Please note that themes may not be accurate._")
...
if page == "See all versions of dataset":
    ...
    st.write("")
    expand = st.expander("*Caviardage* (Main dataset with additionnal features and cleaning)", icon=":material/info:")
    expand.dataframe(clean_df_v2, use_container_width=True)    
 
    expand = st.expander("*Blackout* (cleaned version)", icon=":material/info:")
    expand.dataframe(dp.clean_df, use_container_width=True)

    expand = st.expander("*Blackout* (original dataset)", icon=":material/info:")
...
st.write("###")
st.divider()
st.caption("App by Safya Dev (1703575) - MAT510 Introduction to Data Science © 2025")